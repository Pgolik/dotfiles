...
vim.keymap.set('n', '<leader>.p', require('azure').find_pr,
  { desc = 'Copy file from remote' })
vim.keymap.set('n', '<leader>.t', require('azure').get_remote,
  { desc = 'Copy file from remote' })
vim.keymap.set('n', '<leader>.b',
  ":let @+ ='?path=' . expand('%.') . '&line=' . line('.') . '&lineEnd=' . (line('.') + 1) . '&lineStartColumn=1&lineEndColumn=1&lineStyle=plain&_a=files'<CR>",
  { desc = 'Get current relative path with line' })
